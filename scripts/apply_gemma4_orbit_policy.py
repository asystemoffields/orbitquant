...
def args_from_config(
    config: dict,
    calib_max_length: int | None = None,
    mlp_fold_down_proj: bool | None = None,
) -> SimpleNamespace:
    policy = config["selected_policy"]
    evaluation = config.get("evaluation", {})
    configured_fold = bool(policy.get("mlp_fold_down_proj", False))
    ...
    return SimpleNamespace(
        ...
        mlp_fold_down_proj=configured_fold if mlp_fold_down_proj is None else bool(mlp_fold_down_proj),
        calib_max_length=int(calib_max_length or evaluation.get("calib_max_length") or 192),
    )


def apply_gemma4_orbit_policy(
    model,
    tokenizer,
    config: dict,
    ...
    args = args_from_config(config, mlp_fold_down_proj=mlp_fold_down_proj)
